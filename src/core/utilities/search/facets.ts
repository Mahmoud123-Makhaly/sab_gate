...
export function getFilterExpressionForZeroPrice(value: boolean, currencyCode?: string): string {
  const priceFilterExpression = currencyCode ? `price.${currencyCode}:(0 TO)` : "price:(0 TO)";

  return value ? "" : priceFilterExpression;
}

export function getFilterExpressionForInStock(value: boolean): string {
  return value ? "availability:InStock" : "";
}

export function getFilterExpressionForAvailableIn(value: string[]): string {
  const branches = value;
  return branches.length ? `available_in:"${branches.join('","')}"` : "";
}

export function getFilterExpressionFromFacets(facets: FacetItemType[]): string {
  const result: string[] = [];
