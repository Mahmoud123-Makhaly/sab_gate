  ...
  return (
    <div className="evaluation-feedback">
      <div className="text-end ">
        <i className="fa-solid fa-xmark close-icon text-gray pointer" onClick={() => setIsOpenModal('')}></i>
      </div>
      <div className="feedBack-info text-center">
        <Image src={check} alt="check" width={0} height={0} />
        <h3 className="feedBack-title text-24 text-info my-3">{t('EVALUATION_SEND_SUCCESSFULLY')}</h3>
        <p className="feedBack-desc"> {t('THANKS_MESSAGE')} </p>
        <Link href={'/list/baked'}>
          <ButtonMaker
            text={t('CONTINUE_SHOPPING')}
            design="bg-primary fw-bold mt-3"
            block={true}
